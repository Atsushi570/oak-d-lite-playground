"""
OAK-D Lite 顔認証（SFace + ObjectTracker）
- OAK VPU: 顔検出 + 追尾
        ...
        if in_track and frame is not None:
            ...
            for t in in_track.tracklets:
                ...
                if last_depth_frame is not None:
                    ...
                    y_max = min(dh, cy + margin)
                    x_min = max(0, cx - margin)
                    x_max = min(dw, cx + margin)
                    roi = last_depth_frame[y_min:y_max, x_min:x_max]
                    valid = roi[(roi > 100) & (roi < 4000)]  # 10cm〜4m の範囲のみ有効
                    if len(valid) > 5:
                        dist_mm = int(np.median(valid))
                        dist_label = f"{dist_mm // 1000}.{(dist_mm % 1000) // 100}m"
